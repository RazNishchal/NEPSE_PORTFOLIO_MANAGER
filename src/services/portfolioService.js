...
export const updatePortfolioInDB = async (userId, tx, currentHoldings = {}, marketData = {}) => {
    ...
    updates[holdingPath] = newUnits <= 0 ? null : {
        symbol, companyName, units: newUnits,
    ...
    updates[`users/${userId}/transactions/${newTxKey}`] = {
        symbol, companyName, type: tx.type, units: txUnits, price: txPrice, timestamp: Date.now()
    };
...
const pruneTransactions = async (userId) => {
    ...
    const allTx = [];
    snapshot.forEach((child) => { allTx.push({ key: child.key, ...child.val() }); });
    ...
    if (Object.keys(cleanupUpdates).length > 0) return update(ref(db), cleanupUpdates);
};